...
// --- GÜNCELLENMİŞ TARİH FORMATLAMA FONKSİYONU ---
function formatDateToDDMMYYYY(dateString) {
    if (!dateString) return 'N/A';
    
    // Eğer format zaten GG.AA.YYYY ise direkt döndür
    if (dateString.match(/^\d{2}\.\d{2}\.\d{4}$/)) { return dateString; }
    
    // DateTime string'ini temizle (Örn: '20.10.2023 10:30' -> '20.10.2023')
    const cleanDateStr = dateString.split(' ')[0];
    const parts = cleanDateStr.split('.'); 
    
    // Eğer format GG.AA.YYYY ise, parçalar 3 adet olmalı
    if (parts.length === 3 && parts[2].length === 4) {
        // GG.AA.YYYY formatını koru
        return `${parts[0].padStart(2, '0')}.${parts[1].padStart(2, '0')}.${parts[2]}`;
    }

    try {
        // Eğer hala ayrıştırılamıyorsa, Date objesiyle dener
...
// --- GÜNCELLENMİŞ YENİ KART KONTROL FONKSİYONU ---
function isNew(dateStr) { 
    if (!dateStr || dateStr === 'N/A') return false; 
    
    // dateStr artık sadece GG.AA.YYYY formatında olmalı (formatDateToDDMMYYYY sayesinde)
    const parts = dateStr.split('.');
    if (parts.length !== 3) { 
        console.warn("isNew: Hatalı tarih formatı:", dateStr);
        return false; 
    }
    
    // YYYY, MM (0-indexli), GG formatında Date objesi oluştur
    let date = new Date(Date.UTC(parts[2], parts[1] - 1, parts[0])); 
    
    if (isNaN(date.getTime())) return false; 
    
    // Bugünün başlangıcını al (00:00:00 UTC)
    const now = new Date();
    const todayStart = new Date(Date.UTC(now.getFullYear(), now.getMonth(), now.getDate()));
    
    const diffTime = todayStart.getTime() - date.getTime(); 
    
    // Farkı gün sayısına çevir (1 gün = 86,400,000 milisaniye)
    const diffDays = Math.floor(diffTime / (1000 * 60 * 60 * 24)); 
    
    // 0, 1, 2, 3. gün ise (yani bugün ve önceki 3 gün) "Yeni" sayılır.
...
                        // --- GÜNCELLENMİŞ KIRILIM NOTU STİLİ ---
                        let noteDisplay = item.note ? `<div style="margin-top:5px; padding:5px 10px; background:#fff3f3; border:1px solid #d32f2f; color:#a11616; border-radius:4px; font-size:0.85rem; font-style:normal;">🛑 Kırılım Nedeni: ${item.note}</div>` : '';
...
            </div>
            <div style="font-size:0.8rem; opacity:0.7; color:#1976d2; grid-column: 1 / -1; text-align:center;">
                İlk Log: ${evalData.date} / Değerlendiren: ${evalData.evaluator || 'N/A'}
            </div>
        </div>
    `;

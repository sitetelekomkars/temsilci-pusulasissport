...
// DEĞİŞİKLİK BAŞLANGICI: window.updateRowScore fonksiyonu (Görsel ve Zorunluluk)
window.updateRowScore = function(index, max) {
    ...
    badge.innerText = val;
    // Görsel değişimler ve zorunluluk (required) ekleme
    if (val < max) {
        ...
        noteInput.required = false; // Puan tamsa notu zorunlu olmaktan çıkar
...
// DEĞİŞİKLİK BİTİŞİ: window.updateRowScore fonksiyonu

// DEĞİŞİKLİK BAŞLANGICI: window.recalcTotalScore fonksiyonu (Yüzdelik ve Renk Ataması)
...
// DEĞİŞİKLİK BİTİŞİ: window.recalcTotalScore fonksiyonu
...
"Hatalı giriş!"; 
        ...
        loadingMsg.style.display = "none"; 
        document.querySelector('.login-btn').disabled = false; 
        errorMsg.innerText = "Sunucu yoğun, lütfen tekrar deneyin."; 
        errorMsg.style.display = "block"; 
    });
}

// DEĞİŞİKLİK BAŞLANGICI: checkAdmin fonksiyonu (Menü butonları)
...
// DEĞİŞİKLİK BİTİŞİ: checkAdmin fonksiyonu
...
function loadContentData() { 
    ...
    .then(data => {
        ...
        if (data.result === "success") {
            ...
            const fetchedNews = rawData.filter(i 
=> i.Type.toLowerCase() === 'news').map(i => ({ 
                date: formatDateToDDMMYYYY(i.Date),
                title: i.Title, 
                desc: i.Text, 
                type: i.Category, 
            ...
            document.getElementById('loading').innerHTML = `Veriler alınamadı: ${data.message ||
'Bilinmeyen Hata'}`;
        }
    })
    .catch(error => { 
...
text-align:right;">(Tamamını oku)</div>
            ...
            <div class="card-actions">
                ...
                ${item.code ? `<button class="btn btn-copy" style="background:var(--secondary);
color:#333;" onclick="copyText('${escapeForJsString(item.code)}')">Kod</button>` : ''}
...
// DEĞİŞİKLİK BAŞLANGICI: toggleEditMode fonksiyonu (Menü butonları)
...
// DEĞİŞİKLİK BİTİŞİ: toggleEditMode fonksiyonu
...
gap: 10px;">
                    <input id="swal-new-title" class="swal2-input" style="margin:0;
height:40px; flex-grow:1; border:none; border-bottom:2px solid #eee; padding:0 5px; font-weight:bold; color:#0e1b42;" placeholder="Başlık Giriniz...">
                    <div id="cat-container" style="width: 110px;">${catSelectHTML}</div>
                </div>
                <div class="card-content" style="margin-bottom:10px;">
                    <textarea id="swal-new-text" class="swal2-textarea" style="margin:0;
width:100%; box-sizing:border-box; border:none; resize:none; font-family:inherit; min-height:100px; padding:10px; background:#f9f9f9;" placeholder="İçerik metni..."></textarea>
                </div>
                <div id="script-container" class="script-box" style="padding:0;
border:1px solid #f0e68c;">
                    <textarea id="swal-new-script" class="swal2-textarea" style="margin:0;
width:100%; box-sizing:border-box; border:none; background:transparent; font-style:italic; min-height:80px; font-size:0.9rem;" placeholder="Script metni (İsteğe bağlı)..."></textarea>
                </div>
                <div id="extra-container" class="card-actions" style="margin-top:15px;
display:grid; grid-template-columns: 1fr 1fr; gap:10px;">
                    <div style="position:relative;"><i class="fas fa-code" style="position:absolute;
left:10px; top:10px; color:#aaa;"></i><input id="swal-new-code" class="swal2-input" style="margin:0; height:35px; font-size:0.85rem; padding-left:30px;" placeholder="Kod"></div>
                    <div style="position:relative;"><i class="fas fa-link" style="position:absolute;
left:10px; top:10px; color:#aaa;"></i><input id="swal-new-link" class="swal2-input" style="margin:0; height:35px; font-size:0.85rem; padding-left:30px;" placeholder="Link"></div>
                </div>
...
= document.getElementById('swal-new-cat'); 
            ...
            selectEl.style.height = "30px"; selectEl.style.fontSize = "0.8rem"; selectEl.style.padding = "0 5px";
            ...
            window.toggleAddFields = function() {
                ...
                scriptCont.style.display = 'none'; extraCont.style.display = 'none'; 
                ...
                if (type === 'card') {
                    ...
                    scriptCont.style.display = 'block'; extraCont.style.display = 'grid';
...
document.getElementById('swal-quiz-q').value : '';
            ...
            return { 
                cardType: type, 
                category: type === 'card' ?
document.getElementById('swal-new-cat').value : (type === 'news' ? document.getElementById('swal-news-type').value : ''), 
                title: document.getElementById('swal-new-title').value, 
                text: type === 'quiz' ?
quizQ : document.getElementById('swal-new-text').value, 
                script: (type === 'card' || type === 'sales') ?
document.getElementById('swal-new-script').value : '', 
                code: type === 'card' ?
document.getElementById('swal-new-code').value : '', 
                status: type === 'news' ?
document.getElementById('swal-news-status').value : '',
                link: type === 'card' ?
document.getElementById('swal-new-link').value : '',
                tip: type === 'sport' ?
document.getElementById('swal-sport-tip').value : '',
                detail: type === 'sport' ?
document.getElementById('swal-sport-detail').value : '',
                pronunciation: type === 'sport' ?
document.getElementById('swal-sport-pron').value : '',
                icon: type === 'sport' ?
document.getElementById('swal-sport-icon').value : '',
                date: dateStr,
        ...
        html: `
            <div id="preview-card-edit" class="card ${item.category}" style="text-align:left; box-shadow:none; border:1px solid #e0e0e0; margin-top:10px;">
                <div class="card-header" style="align-items: center; gap: 10px;">
                    <input id="swal-title" class="swal2-input" style="margin:0; height:40px; flex-grow:1; border:none; border-bottom:2px solid #eee; padding:0 5px; 
font-weight:bold; color:#0e1b42;" value="${item.title}" placeholder="Başlık">
                    <div style="width: 110px;">${catSelectHTML}</div>
                </div>
                <div class="card-content" style="margin-bottom:10px;">
                    <textarea id="swal-text" class="swal2-textarea" style="margin:0; width:100%; box-sizing:border-box; border:none; resize:none; font-family:inherit; min-height:120px; padding:10px; background:#f9f9f9;" placeholder="İçerik metni...">${(item.text ||
'').toString().replace(/<br>/g,'\n')}</textarea>
                </div>
                <div class="script-box" style="padding:0; border:1px solid #f0e68c;">
                    <textarea id="swal-script" class="swal2-textarea" style="margin:0; width:100%; box-sizing:border-box; border:none; background:transparent; font-style:italic; min-height:80px; font-size:0.9rem;"
placeholder="Script metni...">${(item.script || '').toString().replace(/<br>/g,'\n')}</textarea>
                </div>
                <div class="card-actions" style="margin-top:15px; display:grid; grid-template-columns: 1fr 1fr; gap:10px;">
                    <div style="position:relative;"><i class="fas fa-code" style="position:absolute; left:10px; top:10px; color:#aaa;"></i><input id="swal-code" class="swal2-input" style="margin:0; height:35px; font-size:0.85rem; padding-left:30px;"
value="${item.code || ''}" placeholder="Kod"></div>
                    <div style="position:relative;"><i class="fas fa-link" style="position:absolute; left:10px; top:10px; color:#aaa;"></i><input id="swal-link" class="swal2-input" style="margin:0; height:35px; font-size:0.85rem; padding-left:30px;"
value="${item.link || ''}" placeholder="Link"></div>
                </div>
            </div>`,
        width: '700px', 
        showCancelButton: true, 
    ...
    const { value: formValues } = await Swal.fire({
        ...
        html: `
            <div class="card" style="text-align:left; border-left: 5px solid var(--primary); padding:15px; background:#f8f9fa;">
                ...
                <input id="swal-pron" class="swal2-input" style="width:100%;
margin-bottom:10px;" value="${s.pronunciation || ''}">
...
''}">
    ...
    if (formValues) {
        ...
        if(formValues[3] !== s.detail) setTimeout(() => sendUpdate(originalTitle, "Detail", formValues[3], 
'sport'), 1000);
...
async function editNews(index) {
    ...
    let statusOptions = `<option value="Aktif" ${i.status !== 'Pasif' ?
'selected' : ''}>Aktif</option><option value="Pasif" ${i.status === 'Pasif' ? 'selected' : ''}>Pasif (Gizle)</option>`;
...
`<div style="color:#e65100; font-weight:bold; margin-bottom:15px;"> 🗣️  Okunuşu: ${sport.pronunciation}</div>` : '';
    Swal.fire({ 
            ...
            filteredEvals.reverse().forEach((evalItem, index) => { 
                ...
                try {
                    ...
                    detailObj.forEach(item => {
                        ...
                        let noteDisplay = item.note ? `<br><em style="color: #d32f2f; font-size:0.8rem;">(Kırılım 
Nedeni: ${item.note})</em>` : '';
...
`<div style="position:absolute; top:10px; right:40px; cursor:pointer; color:#1976d2;" onclick="event.stopPropagation(); editEvaluation('${evalItem.callId}')" title="Değerlendirmeyi Düzenle"><i class="fas fa-edit fa-lg"></i></div>` : '';
                html += `<div class="evaluation-summary" id="eval-summary-${index}" style="position:relative; border:1px solid #ddd; border-left:5px solid ${scoreColor}; padding:15px; margin-bottom:10px; border-radius:6px; background:#fff; cursor:pointer;"
...
${displayLogDate} 
                        ...
                        <span style="font-size:0.9rem; color:#666;">Call ID: ${evalItem.callId ||
'-'}</span> 
                    ...
                    <div 
class="evaluation-details-content" id="eval-details-${index}" style="max-height:0; overflow:hidden; transition:max-height 0.4s ease-in-out; margin-top:0;">
            ...
            listEl.innerHTML = `<p style="color:red; text-align:center;">Veri çekme hatası: ${data.message ||
'Bilinmeyen Hata'}</p>`; 
        }
    } catch(err) {
        loader.style.display = 'none';
...
async function exportEvaluations() {
    ...
    const agentName = targetAgent === 'all' ?
'Tüm Temsilciler' : targetAgent;
    ...
    try {
        const response = await fetch(SCRIPT_URL, { 
            ...
            body: JSON.stringify({ 
                ...
                targetAgent: targetAgent, // 'all' veya temsilci 
adı
                username: currentUser, 
                token: getToken() 
            }) 
...
// DEĞİŞİKLİK BAŞLANGICI: fetchUserListForAdmin fonksiyonu (Yönetim Filtresi)
function fetchUserListForAdmin() {
    return new Promise((resolve) => {
        ...
        .then(data => { 
            if (data.result === "success") { 
                // YENİ EKLENEN FİLTRELEME: "Yönetim" grubunu gizle
...
// DEĞİŞİKLİK BİTİŞİ: fetchUserListForAdmin fonksiyonu
...
// DEĞİŞİKLİK BAŞLANGICI: logEvaluationPopup fonksiyonu (Görsel ve Zorunluluk)
async function logEvaluationPopup() {
    ...
    const { value: formValues } = await Swal.fire({
        ...
        preConfirm: () => {
            ...
            // Kriter bazlı form kontrolü
            if (isCriteriaBased) {
                // Zorunlu Not Kontrolü (Yeni Eklenen)
                let requiredNoteMissing = false;
                criteriaList.forEach((c, i) => {
                    const slider = document.getElementById(`slider-${i}`);
                    const note = document.getElementById(`note-${i}`);
                    // Eğer puan max değilse VE not alanı boşsa
                    if (parseInt(slider.value) < parseInt(slider.max) && !note.value.trim()) {
                        requiredNoteMissing = true;
                    }
                });

                if (requiredNoteMissing) {
                    Swal.showValidationMessage('Tam puan verilmeyen her kriter için Kırılım Nedeni girmek zorunludur.');
                    return false;
                }
                ...
                // Puana göre yüzdelik hesaplama
                const totalMax = criteriaList.reduce((sum, c) => sum + (parseInt(c.points) || 0), 0);
                const finalScore = totalMax > 0 ? Math.round((total / totalMax) * 100) : 0;
                
                return { agentName, agentGroup, callId, callDate: formattedCallDate, score: finalScore, details: JSON.stringify(detailsArr), feedback };
...
// DEĞİŞİKLİK BİTİŞİ: logEvaluationPopup fonksiyonu

// DEĞİŞİKLİK BAŞLANGICI: editEvaluation fonksiyonu (Görsel ve Zorunluluk)
...
selectEl.options[selectEl.selectedIndex] : null;
    ...
    let oldDetails = [];
    try { oldDetails = JSON.parse(evalData.details);
    } catch(e) { oldDetails = []; }
    let contentHtml = `
...
Düzeltme)</div>
        ...
        <div class="eval-header-card">
            <div>
                <label style="font-size:0.8rem; font-weight:bold; color:#555;">Call ID</label>
                <input id="eval-callid" class="swal2-input" style="height:35px; margin:0; width:100%; font-size:0.9rem; background:#eee;"
value="${evalData.callId}" readonly>
            </div>
            <div>
                <label style="font-size:0.8rem; font-weight:bold; color:#555;">Çağrı Tarihi</label>
                <input type="text" class="swal2-input" style="height:35px; margin:0; width:100%; font-size:0.9rem; background:#eee;"
value="${evalData.callDate}" readonly>
            </div>
        </div>
    `;
    if (isCriteriaBased) {
    ...
    const { value: formValues } = await Swal.fire({
        ...
        preConfirm: () => {
            ...
            if (isCriteriaBased) {
                ...
                return { agentName, callId, score: finalScore, details: JSON.stringify(detailsArr), feedback };
...
// DEĞİŞİKLİK BİTİŞİ: editEvaluation fonksiyonu
        ...
        if (isCorrect) { 
            ...
            if(Math.random() > 0.5) { 
                ...
                setTimeout(() => { 
                    ...
                    goalMsg.classList.add('show');
                    Swal.fire({ icon: 'error', title: 'Kaçırdın!', text: `Doğru cevap: ${String.fromCharCode(65 + pCurrentQ.a)}. 
${pCurrentQ.opts[pCurrentQ.a]}`, showConfirmButton: true, timer: 2500, background: '#ef9a9a' }); 
                }, 500);
            } else { 
                ...
                setTimeout(() => { 
                    ...
                    goalMsg.style.color = "#ef5350"; 
                    goalMsg.classList.add('show'); 
                    Swal.fire({ icon: 'error', title: 'Kaçırdın!', text: `Doğru cevap: ${String.fromCharCode(65 + 
pCurrentQ.a)}. ${pCurrentQ.opts[pCurrentQ.a]}`, showConfirmButton: true, timer: 2500, background: '#ef9a9a' }); 
                }, 500);
            } 
        } 
    }, 300); 
...
function finishPenaltyGame() { 
    let title = pScore >= 8 ?
"EFSANE!  🏆 " : (pScore >= 5 ? "İyi Maçtı!  👏 " : "Antrenman Lazım  🤕 ");
...
function renderStep(k){ 
    ...
    if (!s) {
        document.getElementById('wizard-body').innerHTML = `<h2 style="color:red;">HATA: Adım ID'si (${k}) bulunamadı.
Lütfen yöneticinizle iletişime geçin.</h2>`;
        return;
    }
...
' 🛑 ' : (s.result === 'green' ? ' ✅ ' : ' ⚠️ ');
        ...
        h += `<div class="result-box ${c}"><div style="font-size:3rem;margin-bottom:10px;">${i}</div><h3>${s.title}</h3><p>${s.text}</p>${s.script ?
`<div class="script-box">${s.script}</div>` : ''}</div><button class="restart-btn" onclick="renderStep('start')"><i class="fas fa-redo"></i> Başa Dön</button>`;
    } else {
        // Ara Adım

...
let lastEvalOffset = 0;
const EVAL_PAGE_SIZE = 30;
...
async function fetchEvaluationsForFeedback() {
    ...
    try {
        const d = await apiCall("fetchEvaluations", { 
            targetAgent, 
            targetGroup,
            feedbackSection: true // Geri bildirim sekmesi için Feedback/Details içeren geniş veri çek
        });
        if (d.result === "success") {
...
async function fetchEvaluationsForDashboard() {
    ...
    try {
        ...
        const periodSelect = document.getElementById('q-dash-month');
        ...
        const d = await apiCall("fetchEvaluations", { 
            ...
            mini: true // DASH_COLS (Hafif Veri)
        });
    ...
    const { value: formValues } = await Swal.fire({
        ...
        html: `
            <div class="t-modal-grid">
                ...
                <input type="date" id="swal-t-start" class="swal2-input" value="${new Date(Date.now() - (new Date().getTimezoneOffset() * 60000)).toISOString().substring(0, 10)}">
        ...
        <div class="manual-feedback-form">
            ...
            <div class="grid-2-cols">
                ...
                <div class="form-group">
                    ...
                    <input type="date" id="manual-q-date" class="swal2-input" value="${new Date(Date.now() - (new Date().getTimezoneOffset() * 60000)).toISOString().substring(0, 10)}">
                </div>
            </div>
...
async function fetchEvaluationsForAgent(forcedName, silent = false, isLoadMore = false) {
    ...
    try {
        ...
        if (data && data.result === "success") {
            ...
            if (newEvaluations.length === 0 && !isLoadMore) {
                listEl.innerHTML = '<p style="padding:20px; text-align:center; color:#666;">Kayıt yok.</p>';
                return;
            }

            allEvaluationsData = [...allEvaluationsData, ...newEvaluations];
            lastEvalOffset += newEvaluations.length;

            if (silent) return;

            // Render logic
            let listElBuffer = "";
            newEvaluations.forEach((evalItem, index) => {
                const globalIndex = allEvaluationsData.length - newEvaluations.length + index;
                const scoreCircleColor = evalItem.score >= 90 ? '#48bb78' : (evalItem.score >= 70 ? '#ed8936' : '#f56565');

                let editBtn = isAdminMode ? `
                    <div style="display:flex; gap:10px; align-items:center;">
                        <i class="fas fa-pen" style="font-size:0.9rem; color:#718096; cursor:pointer; transition:0.2s;" onmouseover="this.style.color='#3182ce'" onmouseout="this.style.color='#718096'" onclick="event.stopPropagation(); editEvaluation('${evalItem.callId}')"></i>
                        <i class="fas fa-trash-alt" style="font-size:0.9rem; color:#718096; cursor:pointer; transition:0.2s;" onmouseover="this.style.color='#e53e3e'" onmouseout="this.style.color='#718096'" onclick="event.stopPropagation(); deleteEvaluation('${evalItem.id}', '${evalItem.callId}')"></i>
                    </div>
                ` : '';
                ...
                listElBuffer += `
                <div class="eval-card-v2" id="eval-card-${globalIndex}" onclick="newToggleEvaluationDetail(${globalIndex}, '${evalItem.callId}', ${isSeen}, this)">
                    ...
                    <div class="eval-details-pane-v2" id="eval-details-${globalIndex}">
                        <div class="loading-details" style="padding:20px; text-align:center; color:#666;">
                            <i class="fas fa-spinner fa-spin"></i> Detaylar yükleniyor...
                        </div>
                    </div>
                </div>`;
            });
            ...
            if (isLoadMore) {
                listEl.insertAdjacentHTML('beforeend', listElBuffer);
            } else {
                listEl.innerHTML = listElBuffer;
            }

            // Sayfalama Butonu
            const oldBtn = document.getElementById('btn-load-more-eval');
            if (oldBtn) oldBtn.remove();
            
            if (newEvaluations.length === EVAL_PAGE_SIZE) {
                const loadMoreBtn = `
                <div id="btn-load-more-container" style="text-align:center; padding:20px;">
                    <button id="btn-load-more-eval" class="q-btn-v2" style="background:var(--primary); color:white; border:none; padding:8px 20px; border-radius:6px; cursor:pointer;" onclick="fetchEvaluationsForAgent(null, false, true)">
                        <i class="fas fa-chevron-down"></i> Daha Fazla Yükle
                    </button>
                </div>`;
                listEl.insertAdjacentHTML('afterend', loadMoreBtn);
            }
        } else {
            if (!silent) {
                listEl.innerHTML = `<div style="text-align:center; padding:40px; color:#c62828;"><p>Bağlantı Sorunu: ${data.message}</p></div>`;
            }
        }
    } catch (err) {
        console.error(err);
        if (!silent) {
            listEl.innerHTML = `<div style="text-align:center; padding:40px; color:#c62828;"><p>Bağlantı Sorunu</p></div>`;
        }
    }
}

// Yeni Toggle Fonksiyonu (V2)
async function newToggleEvaluationDetail(index, callId, isAlreadySeen, element) {
    ...
    if (isExpanding) {
        ...
        // Lazy Load: Eğer detaylar henüz yüklenmemişse getir
        const evalItem = allEvaluationsData[index];
        if (!evalItem.details) {
            try {
                const res = await apiCall("fetchSingleEvaluation", { id: evalItem.id });
                if (res.result === "success") {
                    evalItem.details = res.evaluation.details;
                    evalItem.feedback = res.evaluation.feedback;
                    evalItem.feedbackType = res.evaluation.feedbackType;
                    evalItem.agentNote = res.evaluation.agentNote;
                    evalItem.managerReply = res.evaluation.managerReply;
                    
                    // Detay Panelini Render et
                    renderEvaluationDetails(index, evalItem);
                }
            } catch (e) {
                console.error("Lazy Load Error:", e);
                detailEl.innerHTML = '<div style="padding:20px; color:red;">Detaylar yüklenemedi.</div>';
            }
        }
...
function renderEvaluationDetails(index, evalItem) {
    const detailEl = document.getElementById(`eval-details-${index}`);
    
    let detailTableHtml = '';
    try {
        let detailObj = evalItem.details;
        if (typeof detailObj === 'string') detailObj = JSON.parse(detailObj);
        ...
        detailTableHtml = `<div class="eval-feedback-box-v2">${escapeHtml(String(evalItem.details))}</div>`;
    }
    ...
    if (agentNote || managerReply) {
        ...
        if (managerReply) {
            notesDisplay += `<div class="eval-interaction-bubble bubble-manager" style="align-self: flex-end;">
...
function fetchUserListForAdmin() {
    const cacheKey = 'pusula_admin_userlist';
    const cached = sessionStorage.getItem(cacheKey);
    if (cached) {
        adminUserList = JSON.parse(cached);
        return Promise.resolve(adminUserList);
    }
    ...
    return new Promise((resolve) => {
        apiCall("getUserList", {}).then(data => {
            if (data.result === "success") {
                ...
                sessionStorage.setItem(cacheKey, JSON.stringify(adminUserList));
...
function fetchCriteria(groupName) {
    const cacheKey = `pusula_criteria_${groupName}`;
    const cached = sessionStorage.getItem(cacheKey);
    if (cached) {
    ...
    return new Promise((resolve) => {
        apiCall("getCriteria", { group: groupName }).then(data => {
            if (data.result === "success") {
                ...
                sessionStorage.setItem(cacheKey, JSON.stringify(criteria));
                resolve(criteria);
            } else resolve([]);
        }).catch(err => resolve([]));
    });
...
async function logEvaluationPopup() {
    ...
    const contentHtml = `
        <div class="eval-modal-v2">
            <div class="eval-form-header">
                ...
                <button type="button" id="v2-reset-btn" class="eval-btn-v2 bad" style="margin-left:auto; height:fit-content; padding:8px 16px; font-weight:800; border-radius:8px; box-shadow:0 2px 4px rgba(229,62,62,0.2);">SIFIRLA</button>
            </div>
            ...
            <div class="eval-form-grid">
                ...
                <div class="eval-input-group">
                    ...
                    <input type="date" id="eval-calldate" class="eval-input-v2" value="${new Date(Date.now() - (new Date().getTimezoneOffset() * 60000)).toISOString().substring(0, 10)}">
                </div>
            </div>
    ...
    const { value: formValues } = await Swal.fire({
        ...
        didOpen: () => {
            if (isTelesatis) window.v2_recalc();
            else if (isChat) window.v2_recalc();

            const resetBtn = document.getElementById('v2-reset-btn');
            if (resetBtn) {
                resetBtn.addEventListener('click', () => {
                    if (confirm("Emin misiniz? Tüm kriterler 0 puana çekilecek ve çağrı sıfırlanacaktır!")) {
                        if (isTelesatis) window.v2_resetAllCriteria('telesatis', criteriaList);
                        else if (isChat) window.v2_resetAllCriteria('chat', criteriaList);
                        else window.v2_resetAllCriteria('chat', criteriaList); // Fallback
                    }
                });
            }
        },
...
async function editEvaluation(targetCallId) {
    ...
    // Lazy Load: Eğer detaylar henüz yüklenmemişse (Diyet v2) getir
    if (!evalData.details) {
        Swal.fire({ title: 'Yükleniyor...', didOpen: () => Swal.showLoading() });
        const res = await apiCall("fetchSingleEvaluation", { id: evalData.id });
        Swal.close();
        if (res.result === "success") {
            evalData.details = res.evaluation.details;
            evalData.feedback = res.evaluation.feedback;
            evalData.feedbackType = res.evaluation.feedbackType;
        } else {
            Swal.fire('Hata', 'Detaylar alınamadı.', 'error');
            return;
        }
    }
    ...
    const contentHtml = `
        <div class="eval-modal-v2">
            <div class="eval-form-header" style="border-bottom-color:#1976d2;">
                <div class="eval-form-user">
                    <div class="eval-form-avatar" style="background:#1976d2;">${agentName.charAt(0).toUpperCase()}</div>
                    <div>
                        <div style="font-size:0.8rem; color:#718096; font-weight:700;">DÜZENLENEN</div>
                        <div style="font-size:1.1rem; font-weight:800; color:#1976d2;">${agentName}</div>
                    </div>
                </div>
                <button type="button" id="v2-reset-btn" class="eval-btn-v2 bad" style="margin-left:auto; height:fit-content; padding:8px 16px; font-weight:800; border-radius:8px; box-shadow:0 4px 6px rgba(229,62,62,0.2);">SIFIRLA</button>
                <div class="eval-form-score-box">
                    <div class="eval-form-score-val" id="v2-live-score">${evalData.score}</div>
                    <div class="eval-form-score-label">MEVCUT PUAN</div>
                </div>
            </div>
...
// syncTelesalesScriptsToSheet removed - migrated to PusulaDB
...
async function deleteEvaluation(id, callId) {
    if (!isAdminMode) return;
    
    const result = await Swal.fire({
        title: 'Silinsin mi?',
        text: `#${callId} numaralı değerlendirme kalıcı olarak silinecek. Bu işlem geri alınamaz!`,
        icon: 'warning',
        showCancelButton: true,
    ...
    if (result.isConfirmed) {
        ...
        try {
            const { error } = await window.PusulaDB.quality.deleteEvaluation(id);
            if (!error) {
                Swal.fire({ icon: 'success', title: 'Silindi', text: 'Değerlendirme başarıyla silindi.', timer: 1500, showConfirmButton: false });
                // Listeyi yenile
                if (typeof fetchEvaluationsForAgent === 'function') {
                    // Mevcut ajanın listesini tekrar çek
                    const currentAgent = document.getElementById('q-agent-select')?.value || 'all';
                    fetchEvaluationsForAgent(currentAgent === 'all' ? undefined : currentAgent);
                }
            } else {
                throw error;
            }
        } catch (e) {
            console.error("Delete eval error:", e);
            Swal.fire('Hata', 'Değerlendirme silinemedi: ' + e.message, 'error');
        }
    }
}

...
function switchTelesalesTab(tab) {
    ...
    const topTitle = document.querySelector('#telesales-fullscreen .q-topbar .q-title');

    if (wrap) {
        wrap.classList.toggle('competition-mode', tab === 'competition');
        if (tab === 'competition') {
            wrap.classList.add('arena-focus-mode');
            wrap.classList.add('arena-bottom-hidden');
        } else {
            wrap.classList.remove('arena-focus-mode');
            wrap.classList.remove('arena-bottom-hidden');
        }
    }
    if (body) body.classList.toggle('competition-mode-open', tab === 'competition');

    if (topTitle) {
        topTitle.textContent = tab === 'competition' ? 'Satış Serüveni' : 'TeleSatış';
    }
        ...
        <div style="display:flex;gap:10px;align-items:center;justify-content:space-between;margin-top:10px">
          ...
          ${(isAdminMode && isEditingActive) ? `
            <div style="display:flex;gap:8px">
              ...
              <button class="x-btn x-btn-admin" onclick="event.stopPropagation(); deleteTelesalesScript('${s.id}', ${i});"><i class="fas fa-trash"></i></button>
            </div>
          ` : ``}
        </div>
      </div>
...
// Local storage helpers removed for PusulaDB integration
...
async function addTelesalesScript() {
    const { value: res } = await Swal.fire({
        ...
        preConfirm: () => {
            ...
            return { Type: 'script', Title: title || 'Script', Text: text };
        }
    });
    ...
    if (res) {
        Swal.fire({ title: 'Kaydediliyor...', didOpen: () => Swal.showLoading(), showConfirmButton: false });
        try {
            const { data, error } = await window.PusulaDB.content.upsertContent(res);
            if (!error) {
                // Global listeye ekle ve yeniden çiz (Veya tam yeniden yükleme tetikle)
                if (typeof window.fetchMainData === 'function') await window.fetchMainData();
                renderTelesalesScripts();
                Swal.fire({ icon: 'success', title: 'Eklendi', timer: 1200, showConfirmButton: false });
            } else {
                throw error;
            ...
            Swal.fire('Hata', 'Veri tabanına kaydedilemedi: ' + e.message, 'error');
        }
    }
}

async function editTelesalesScript(id) {
    const s = window.salesScripts.find(x => String(x.id) === String(id));
    if (!s) return;
    const { value: res } = await Swal.fire({
        ...
        preConfirm: () => {
            ...
            const text = (document.getElementById('ts-text').value || '').trim();
            if (!text) return Swal.showValidationMessage("Script metni zorunlu");
            return { id: s.id, Type: 'script', Title: title || 'Script', Text: text };
        }
    });

    if (res) {
        Swal.fire({ title: 'Güncelleniyor...', didOpen: () => Swal.showLoading(), showConfirmButton: false });
        try {
            const { error } = await window.PusulaDB.content.upsertContent(res);
            if (!error) {
                if (typeof window.fetchMainData === 'function') await window.fetchMainData();
                renderTelesalesScripts();
            ...
            Swal.fire('Hata', 'Güncellenemedi: ' + e.message, 'error');
        }
    }
}
async function deleteTelesalesScript(id, idx) {
    const res = await Swal.fire({ title: "Silinsin mi?", text: "Bu script kalıcı olarak silinecek.", icon: "warning", showCancelButton: true, confirmButtonText: "Sil", cancelButtonText: "Vazgeç" });
    if (res.isConfirmed) {
        Swal.fire({ title: 'Siliniyor...', didOpen: () => Swal.showLoading(), showConfirmButton: false });
        try {
            const { error } = await window.PusulaDB.content.deleteContent(id);
            if (!error) {
                window.salesScripts.splice(idx, 1);
                renderTelesalesScripts();
                Swal.fire({ icon: 'success', title: 'Silindi', timer: 1000, showConfirmButton: false });
            } else {
                throw error;
...
window.toggleArenaFocusMode = function () {
    const wrap = document.getElementById('telesales-fullscreen');
    if (!wrap) return;
    wrap.classList.toggle('arena-focus-mode');
};

window.toggleArenaBottomPanels = function () {
    const wrap = document.getElementById('telesales-fullscreen');
    if (!wrap) return;
    wrap.classList.toggle('arena-bottom-hidden');
};

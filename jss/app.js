...
async function fetchGSheetRawTSV(url) {
    return new Promise(async (resolve, reject) => {
        console.log("[Pusula TSV] Ham veri motoru v6.6 aktif (CORS Proxy):", url);
        const idMatch = url.match(/\/d\/([a-zA-Z0-9-_]+)/);
        ...
        try {
            ...
            for(let i=0; i<8; i++) {
                ...
                if (rowStr.includes('event') || rowStr.includes('end time') || rowStr.includes('tsi')) {
                    idxHeader = i; break;
                }
            }

            const header = tsvRows[idxHeader].map(h => h.toLowerCase());
            const idxTitle = header.findIndex(h => h.includes('event') || h.includes('karşılaşma')) || 1;
            const idxTime = header.findIndex(h => h.includes('ko/') || h.includes('ko start') || h.includes('tsi')) || 3;
            const idxNote = header.findIndex(h => h.includes('end time') || h.includes('notlar')) || 4;
            const idxSpiker = header.findIndex(h => h.includes('announcer') || h.includes('spiker')) || 5;

            let lastDate = "";
            const rows = tsvRows.slice(idxHeader + 1).map((cells, rIdx) => {
                if (cells[0]) lastDate = cells[0];
                const rowObj = {
                    col_0: cells[0] || lastDate,
                    col_1: cells[idxTitle] || "",
                    col_2: cells[idxTime-1] || "",
                    col_3: cells[idxTime] || "",
                    col_4: (cells[idxNote] || "").trim(),
                    col_5: (cells[idxSpiker] || "").trim(),
                    _isSheet: true
                };
                const isTrash = /^\d{1,2}:\d{2}(:\d{2})?$/.test(rowObj.col_4) && !/[a-z]/.test(rowObj.col_4.toLowerCase());
                if (isTrash) rowObj.col_4 = "";
                return rowObj;
            }).filter(r => r.col_1 && r.col_1.length > 2);

            console.log(`[Pusula TSV] v6.6 Başarılı: ${rows.length} kayıt işlendi.`);
            resolve(rows);
        } catch (err) {
            reject(err);
        }
...
function normalizeKeys(obj) {
    ...
    let colENote = ""; // E Sütunu Notu için geçici saklama
    ...
    // 🔍 YAYIN AKIŞI NOTLARINI KESİNLEŞTİR (CSV Hibrit Kurtarma)
    // Eğer obj.col_4 (CSV) varsa ve colENote (JSONP) boşsa, CSV'den kurtarılan veriyi al.
    if (!colENote) {
        colENote = String(obj['col_4'] || '').trim();
    }
    
    // Geçersiz değerleri (-, null, ---) temizle ve details alanına bas
    if (colENote && !['-', '---', 'null'].includes(colENote)) {
        n.details = colENote;
    } else if (!n.details) {
        n.details = "";
    }
    // --- ÖZEL MANTIKLAR (Anti-Grafiti Enhanced) ---
    const matchTitle = String(n.match || n.event || n.title || '').toLowerCase();
    const spikerText = String(obj['col_5'] || n.announcer || '').trim();
    const allValuesText = Object.values(obj).join(' ').toLowerCase();

    // 🕵️ İptal Kontrolü: Daha spesifik arama yap (Tüm row'u join etmek tehlikeli olabilir)
    // Sadece Title, Time ve Announcer sütunlarında iptal anahtar kelimesini ara
    const scanText = (matchTitle + " " + spikerText).toLowerCase();
    ...
    if (isCancelled) {
        n.isCancelled = true;
        n.dateISO = "2099-12-31"; // İptalleri doğrudan BİLGİ/Notlar sekmesine at
        const prefix = isRiskli ? `(RİSKLİ) ` : `(İPTAL) `;
        if (n.match && !String(n.match).includes(prefix)) n.match = prefix + n.match;
        if (n.event && !String(n.event).includes(prefix)) n.event = prefix + n.event;
        if (n.title && !String(n.title).includes(prefix)) n.title = prefix + n.title;
    }
    ...
    // StartEpoch hesaplama (Yayın Akışı zamanlama için)
    let dVal = n.date || n.dateISO || obj['DATE'] || obj['TARİH'] || obj['col_0'];
    let tVal = n.time || n._startTime || obj['STREAM START TSI'] || obj['START_TIME_TSI'] || obj['col_2'] || obj['col_3'];
        ...
        try {
            ...
            if (datePart.includes('.')) {
                ...
                if (parts.length >= 2) {
                    ...
                    let y = parts[2] ? parts[2].split(' ')[0].trim() : '2026';
                    if (y.length === 2) y = '20' + y;
                    parsedDateISO = `${y}-${m}-${d}`;
                }
            } else if (dayMatch && monthMatch) {
                const day = dayMatch[0].padStart(2, '0');
                const monthIdx = monthsTr.indexOf(monthMatch[0].toLowerCase());
                parsedDateISO = `2026-${String(monthIdx + 1).padStart(2, '0')}-${day}`;
            } else if (datePart.includes('-')) {
                parsedDateISO = datePart.split('T')[0];
            }
            if (parsedDateISO) {
                n.dateISO = parsedDateISO;
                const timePart = String(tVal).trim().length === 5 ? `${tVal}:00` : tVal;
                const dt = new Date(`${parsedDateISO}T${timePart}`);
                    ...
                    try {
                        ...
                        results.forEach((res, index) => {
                            if (res.status === 'fulfilled') {
                                ...
                                hasSuccess = true;
                                console.log(`[Pusula Debug] Tablo ${index + 1} başarıyla çekildi (TSV).`);
                            } else {
                                console.warn(`[Pusula Debug] Tablo ${index + 1} çekilemedi:`, res.reason);
                            }
                        });

                        if (!hasSuccess) throw new Error("Hiçbir onaylı E-Tablo verisi alınamadı.");

                        const normalizedCount = combinedItems.length;
                        console.log(`[Pusula Debug] Tüm veriler birleştirildi. Toplam: ${normalizedCount}.`);
                        return { result: "success", items: combinedItems.map(normalizeKeys), source: "sheet", count: normalizedCount };
                    } catch (e) {

...
async function fetchCSVAsJSON(url) {
    ...
    try {
        ...
        if (e.message.indexOf('fetch') > -1 || e.message.indexOf('CORS') > -1) {
            Swal.fire("Bağlantı Hatası", "Google Sheets'e erişilemedi. Lütfen dosyanızın 'Web'de Yayınla' yapıldığından emin olun.", "error");
        }
        throw e;
    }
}
...
async function fetchGSheetJSONP(url) {
    return new Promise((resolve, reject) => {
        console.log("[Pusula JSONP] İstek başlatıldı (CORS bypass):", url);
        const idMatch = url.match(/\/d\/([a-zA-Z0-9-_]+)/);
        if (!idMatch) return reject(new Error("Geçersiz Google Sheets linki."));
        
        const sheetId = idMatch[1];
        const gidMatch = url.match(/gid=([0-9]+)/);
        const gid = gidMatch ? gidMatch[1] : 0;
        
        const callbackName = 'gsheet_cb_' + Math.floor(Math.random() * 1000000);
        const script = document.createElement('script');
        
        const timeout = setTimeout(() => {
            reject(new Error("Bağlantı zaman aşımına uğradı (CORS veya Link Hatası)"));
            cleanup();
        }, 10000);
        ...
        window[callbackName] = (response) => {
            cleanup();
            console.log("[Pusula JSONP] Yanıt alındı.");
            try {
                if (response.status === 'error') {
                    throw new Error(response.errors[0].detailed_message || "Veri çekilemedi.");
                }
                const table = response.table;
                let headers = table.cols.map(c => (c.label || '').trim());
                let rowData = table.rows;

                // Dinamik Başlık Tespiti: Eğer ilk satırlarda "EVENT" veya "DATE" varsa o satırı başlık yap
                let headerFoundIndex = -1;
                for (let i = 0; i < Math.min(rowData.length, 5); i++) {
                    const r = rowData[i];
                    if (!r || !r.c) continue;
                    const looksLikeHeader = r.c.some(cell => {
                        const val = String(cell ? (cell.v || cell.f || '') : '').toUpperCase();
                        return val.includes('EVENT') || val.includes('DATE') || val.includes('MAÇ') || val.includes('SAAT');
                    });
                    if (looksLikeHeader) {
                        headerFoundIndex = i;
                        break;
                ...
                if (headerFoundIndex !== -1) {
                    console.log("[Pusula JSONP] Gerçek başlık satırı bulundu (Index:", headerFoundIndex, ")");
                    headers = rowData[headerFoundIndex].c.map((cell, idx) => {
                        return String(cell ? (cell.v || cell.f || '') : '').trim() || `col_${idx}`;
                    });
                    console.log("[Pusula JSONP] Yeni Başlıklar:", headers);
                    rowData = rowData.slice(headerFoundIndex + 1);
                }

                const rows = rowData.map((r, rIdx) => {
                    const rowObj = {};
                    if (!r || !r.c) return rowObj;
                    r.c.forEach((cell, idx) => {
                        let val = cell ? (cell.f || cell.v || '') : '';
                        
                        // Eğer Google amca Date(1899,...) gönderirse temizle
                    ...
                    rowObj._isSheet = true; // Bayrak ekle
                    if (rIdx < 2) console.log(`[Pusula JSONP] Örnek Satır ${rIdx}:`, rowObj);
                    return rowObj;
                });

                // Hata Ayıklama için: Eğer admin ise ve veri gelmediyse küçük bir uyarı ver
                if (rows.length > 0 && (typeof isAdmin !== 'undefined' && isAdmin)) {
                    const sample = JSON.stringify(rows[0]).slice(0, 150);
                    console.log("[Pusula Debug] İlk Satır Objesi:", rows[0]);
                }

                console.log("[Pusula JSONP] Toplam ayrıştırılan nesne:", rows.length);
                resolve(rows);
            } catch (err) {
                reject(err);
            }
        ...
        script.onerror = () => {
            cleanup();
            reject(new Error("Script yükleme hatası. Linki veya internetinizi kontrol edin."));
        };
...
function normalizeKeys(obj) {
    ...
    Object.keys(obj).forEach(k => {
        ...
        // Saf GSheet verisi ise sadece özel mappingleri kullan, genel mappingleri (n[lower]=val) atla
        if (!obj._isSheet) {
            n[k] = obj[k]; // Orijinal key'i koru
            const lower = k.toLowerCase().replace(/\s+/g, '');
            n[lower] = val; // Lowercase versiyonu her zaman ekle
        }
        ...
        // DATE / TARİH (Pos: 0)
        // --- KESİN KONUM-BAZLI EŞLEŞTİRME (Kullanıcı Tanımı: A=Date, B=Match, D=Time, E=Detail) ---
        const allKeys = Object.keys(obj);
        const colIdx = k.startsWith('col_') ? parseInt(k.split('_')[1]) : allKeys.indexOf(k);
        ...
        if (colIdx === 0 || isDate) {
            ...
            if (val) { n.match = val; n.event = val; }
        } else if (colIdx === 3) {
            if (val) n.time = val; 
        } else if (isTime && !n.time) {
            n.time = val;
        } 
        
        // E Sütunu (4): End Time / Detay
        if (colIdx === 4) {
            if (val) n.details = val;
        } else if (kk.includes('END TIME') && val && !n.details) {
            n.details = val;
        }
        
        // F Sütunu (5): Spiker (Strict)
        if (colIdx === 5 || (kk.includes('ANNOUNCER') && val)) {
            if (val && val !== '-' && val !== 'null') n.announcer = val;
        }

        // Fallback: Eğer hala isim boşsa ve bu kolon uzun bir metinse
        if (val && val.length > 8 && !n.match && !isDate && !isTime) {
            n.match = val; n.event = val;
        }

        // --- İPTAL DENETİMİ ---
        const isCancelled = String(n.match || '').toLowerCase().includes('iptal') || 
                            String(n.match || '').toLowerCase().includes('yok') ||
                            String(n.details || '').toLowerCase().includes('çıkartıldı') ||
                            String(n.details || '').toLowerCase().includes('exxen') ||
                            String(obj['col_6'] || '').toLowerCase().includes('iptal') ||
                            String(obj['col_5'] || '').trim() === '-';
        
        if (isCancelled && n.match) {
            n.isCancelled = true;
            if (!n.match.includes('İPTAL')) n.match = `(İPTAL) ${n.match}`;
        }
        ...
        let tVal = n.time || obj['STREAM START TSI'] || obj['START_TIME_TSI'] || obj['col_2'];

        if (dVal && tVal) {
            try {
                let datePart = String(dVal).trim();
                console.log("[Pusula Debug] Ayıklanan Tarih Stringi:", datePart);
                
                // Turkish Date Fix (e.g., "24 Mart Salı")
                const monthsTr = ["ocak", "şubat", "mart", "nisan", "mayıs", "haziran", "temmuz", "ağustos", "eylül", "ekim", "kasım", "aralık"];
                
                // Sayı ve Ay ismini brute-force ara
                const dayMatch = datePart.match(/\d+/);
                const monthMatch = datePart.match(/(ocak|şubat|mart|nisan|mayıs|haziran|temmuz|ağustos|eylül|ekim|kasım|aralık)/i);
                
                if (dayMatch && monthMatch) {
                    const day = dayMatch[0].padStart(2, '0');
                    const mName = monthMatch[0].toLowerCase();
                    const monthIdx = monthsTr.indexOf(mName);
                    const y = new Date().getFullYear();
                    const m = String(monthIdx + 1).padStart(2, '0');
                    datePart = `${y}-${m}-${day}`;
                    console.log("[Pusula Debug] Brute Force Tarih:", datePart);
                ...
                if (!isNaN(dt.getTime())) {
                    ...
                    // Tarih geçersizse (LaLiga Yok vs.) en sona at
                    n.dateISO = "2099-12-31";
                }
            } catch (e) {
                n.dateISO = "2099-12-31";
            }
    ...
    try {
        switch (action) {
            ...
            case "getBroadcastFlow": {
                // Öncelik: Merkezi Veritabanı (HomeBlocks) -> Eskiden localStorage idi
                const sheetUrl = (homeBlocks['broadcast_url']?.content || localStorage.getItem("pusula_broadcast_url") || "").trim();
                
                if (sheetUrl && sheetUrl.startsWith("http")) {
                    console.log("[Pusula Debug] E-Tablo kaynağı aktif (JSONP Deneniyor):", sheetUrl);
                    try {
                        const items = await fetchGSheetJSONP(sheetUrl);
                        const normalizedCount = items.length;
                        console.log("[Pusula Debug] Veri başarıyla çekildi. Normalizasyon başlıyor...");
                        return { result: "success", items: items.map(normalizeKeys), source: "sheet", count: normalizedCount };
                    } catch (e) {
                        console.warn("[Pusula] Sheet fetch failed, falling back to Supabase.", e);
                    }
                }
                ...
                return { result: "success", items: (data || []).map(normalizeKeys), source: "db" };
            }
            case "uploadImage":
...
async function queryIP() {
    const input = document.getElementById('ip-input-field');
    const resBox = document.getElementById('ip-result-box');
    if (!input || !resBox) return;
    const ip = input.value.trim();
    if (!ip) return Swal.fire("Hata", "Lütfen bir IP adresi girin.", "warning");

    resBox.innerHTML = "Sorgulanıyor...";
    resBox.style.display = "block";

    try {
        const res = await fetch(`https://api.ipapi.is/?ip=${ip}`);
        const data = await res.json();
        
        if (data.error) {
            resBox.innerHTML = `<span style="color:red">Hata: ${data.error}</span>`;
            return;
        }

        const isVpn = (data.is_vpn || data.is_proxy || data.is_tor || data.is_datacenter) ? 'EVET (⚠️)' : 'HAYIR';
        const vpnColor = isVpn.includes('EVET') ? '#dc2626' : '#059669';

        resBox.innerHTML = `
            <div style="margin-top:10px; font-size:0.85rem; line-height:1.6; border-top:1px solid #eee; padding-top:10px;">
                <div style="display:flex; justify-content:space-between;"><span>🌍 Konum:</span> <strong>${data.location?.country || '-'}, ${data.location?.city || '-'}</strong></div>
                <div style="display:flex; justify-content:space-between;"><span>📡 ISS:</span> <strong style="max-width:180px; text-align:right; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${data.company?.name || data.asn?.org || '-'}</strong></div>
                <div style="display:flex; justify-content:space-between;"><span>🔒 VPN/Proxy:</span> <strong style="color:${vpnColor}">${isVpn}</strong></div>
                <div style="display:flex; justify-content:space-between;"><span>🕒 Saat:</span> <strong>${data.location?.local_time ? data.location.local_time.split('T')[1].substring(0,5) : '-'}</strong></div>
            </div>
        `;
    } catch (e) {
        resBox.innerHTML = `<span style="color:red">Sorgu sırasında hata oluştu. (Tarayıcı kısıtlaması veya Limit)</span>`;
    }
}

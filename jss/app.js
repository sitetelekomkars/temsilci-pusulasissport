        ...
        try {
            ...
            for (let i = 0; i < Math.min(tsvRows.length, 100); i++) {
                ...
                if (foundTitle > -1 || foundTime > -1) {
                    ...
                    idxTime = foundTime > -1 ? foundTime : 3;
                    idxNote = foundNote > -1 ? foundNote : 4;
                    idxSpiker = foundSpiker > -1 ? foundSpiker : 5;
                    console.log(`[Pusula] Başlıklar tespit edildi (v8.7.4): T:${idxTitle} Saat:${idxTime} N:${idxNote} S:${idxSpiker}`);
                    break;
                }
            }
    ...
    try {
        switch (action) {
            ...
            case "getBroadcastFlow": {
                // 🧠 v8.7.3 Akıllı Önbellek (Yama Sonrası Temizlik)
                const nowTS = Date.now();
                if (window._bfCache && (nowTS - window._bfCache.ts < 120000) && window._bfCache.v === "8.7.6") {
                    console.log("[Pusula] Yayın akışı önbellekten yüklendi (v8.7.6).");
                ...
                if (urls.length > 0) {
                    ...
                    try {
                        ...
                        // Önbelleğe al (v8.7.6)
                        window._bfCache = { ts: nowTS, data: finalItems, v: "8.7.6" };
                ...
                // Böylece kullanıcı tekrar girdiğinde "boş önbelleğe" takılı kalmaz.
                if (dbItems.length === 0) {
                     window._bfCache = { ts: nowTS - 110000, data: [], v: "8.7.6" }; // 10 saniye sonra expire olsun
                } else {
                     window._bfCache = { ts: nowTS, data: dbItems, v: "8.7.6" };
                }
...
// 🚀 v8.7.2 Arka Plan Ön-Yükleme (Işık Hızı İçin)
// Sayfa açıldıktan 12 saniye sonra verileri sessizce çeker ve önbelleğe atar.
setTimeout(() => { 
    console.log("[Pusula] Arka plan yayın akışı verileri ön-yükleniyor..."); 
    apiCall("getBroadcastFlow").catch(() => {}); 
}, 12000);

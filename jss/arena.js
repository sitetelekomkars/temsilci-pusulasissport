                        ...
                        ${pendingRows.map(m => {
                            ...
                            return `
                                <tr style="border-bottom:1px solid #eee;">
                                    ...
                                    <td style="padding:10px;">${safeEsc(task ? task.task_name : 'Silinmiş Görev')}</td>
                                    <td style="padding:10px; display:flex; gap:5px;">
                                        <button onclick="handleMoveAction(${m.id}, 'approved')" class="x-btn x-btn-primary" style="padding:5px 10px; font-size:0.7rem; background:#10b981;">Onayla</button>
                                        <button onclick="handleMoveAction(${m.id}, 'rejected')" class="x-btn" style="padding:5px 10px; font-size:0.7rem; background:#ef4444; color:white;">Reddet</button>
...
function a2PanelMarkup(st) {
    ...
    const actionState = st.isFinished
        ...
        : st.pendingMove
            ? '<div class="arena2-chip arena2-chip-warn">Bu kutu için admin onayı bekleniyor</div>'
            : '<div class="arena2-chip arena2-chip-info">Hazır olduğunda görevi bildir</div>';
    ...
    return `
        <div style="display:flex; align-items:center; justify-content:space-between; gap:10px; margin-bottom:8px;">
            ...
            <div style="font-size:.72rem; color:#93c5fd; padding:5px 10px; border-radius:999px; background:rgba(37,99,235,.14); border:1px solid rgba(96,165,250,.18);">
                v${safeEsc(window.ARENA2_VERSION || '9.2.0')}
            </div>
        </div>
        ...
        <div style="padding:12px; border-radius:16px; background:linear-gradient(180deg, rgba(124,58,237,.18), rgba(15,23,42,.24)); border:1px solid rgba(168,85,247,.24);">
            ...
            <div style="margin-top:8px; font-size:.95rem; font-weight:800; line-height:1.35;">
                ${safeEsc(taskName)}
            </div>
        </div>
...
window.addSpecialTaskType = async function() {
    const result = await Swal.fire({
        title: 'Özel Kutu Tanımı Ekle',
        html: `
            <div style="text-align:left">
                <div style="margin-bottom:10px;">
                    <label style="display:block; margin-bottom:6px;">Tip</label>
                    <select id="arena11-special-type" style="width:100%; padding:10px; border-radius:10px; background:#0f172a; color:#fff; border:1px solid #334155;">
                        <option value="reward">🎁 Hediye</option>
                        <option value="surprise">❓ Sürpriz Soru</option>
                        <option value="bonus">⚡ Bonus Adım</option>
                        <option value="penalty">⚠️ Ceza</option>
                    </select>
                </div>
                <div style="margin-bottom:10px;">
                    <label style="display:block; margin-bottom:6px;">Ad</label>
                    <input id="arena11-special-name" value="Özel Kutu" style="width:100%; padding:10px; border-radius:10px; background:#0f172a; color:#fff; border:1px solid #334155;" />
                </div>
                <div>
                    <label style="display:block; margin-bottom:6px;">Adım</label>
                    <input id="arena11-special-steps" type="number" min="1" max="5" value="1" style="width:100%; padding:10px; border-radius:10px; background:#0f172a; color:#fff; border:1px solid #334155;" />
                </div>
            </div>
        `,
        showCancelButton: true,
        ...
        color: '#fff',
        preConfirm: () => ({
            type: document.getElementById('arena11-special-type')?.value || 'reward',
            name: document.getElementById('arena11-special-name')?.value || 'Özel Kutu',
            steps: Math.max(1, Number(document.getElementById('arena11-special-steps')?.value || 1))
        })
    });
    ...
    const ins = await sb.from('competition_config').insert({
        task_name: v.name,
        steps: v.steps,
        type: v.type
    }).select();
    ...
    if (ins && ins.error) {
        return Swal.fire('Hata', ins.error.message || 'Özel kutu eklenemedi.', 'error');
    }

    await syncCompetitionData();
    return Swal.fire('Kaydedildi', 'Özel kutu tanımı eklendi.', 'success');
};
...
window.a2HumanizeNote = function(note) {
    ...
    if (type === 'submission') {
        return `${box}. Kutu görev bildirimi yapıldı: ${taskName}`;
    } else if (type === 'override') {
        return `${box}. Kutu görevi değiştirildi -> Yeni Görev: ${taskName}`;
    }

    // Bilinmeyen veya farklı bir tip ise etiketleri temizleyip dön
    return s.replace(/\[ARENA2\]/g, '').replace(/\[[^\]]+:[^\]]+\]/g, '').trim() || s;
};

window.a2ArenaRows = function() {
    return (competitionMoves || []).filter(m => String(m?.admin_note || '').includes('[ARENA2]'));
};

window.a2RowsByType = function(teamKey, type) {
    return window.a2ArenaRows().filter(m => {
        const note = String(m?.admin_note || '');
        return note.includes(`[TYPE:${type}]`) && window.a2TagVal(note, 'TEAMKEY') === teamKey;
    });
};
...
window.a2BuildSegmentsForFlow = function(flow) {
...
window.a2BuildSequence = function(teamKey) {
    const seq = [];
    const segments = window.a2BuildSegmentsForFlow(window.a2EffectiveFlow(teamKey));
...
window.a2ApprovedBoxCount = function(teamKey) {
    const approved = window.a2RowsByType(teamKey, 'submission')
        .filter(m => String(m?.status || '').toLowerCase() === 'approved')
        .sort((a, b) => new Date(a?.approved_at || a?.created_at || 0).getTime() - new Date(b?.approved_at || b?.created_at || 0).getTime());

    let furthest = 0;
    approved.forEach(m => {
        const note = String(m?.admin_note || '');
        let endBox = Number(window.a2TagVal(note, 'END') || 0);
        const boxNo = Number(window.a2TagVal(note, 'BOX') || 0);
        const seq = window.a2SequenceBox(teamKey, boxNo);
        if (!endBox && seq) endBox = Number(seq.segment_end || boxNo);

        const tType = seq ? seq.task_type : 'normal';
        if (tType === 'bonus' || tType === 'bonus_step') endBox = Math.min(A2_TOTAL_BOXES, endBox + 1);
        if (tType === 'penalty') endBox = Math.max(0, boxNo - 1);

        furthest = Math.max(furthest, endBox);
    });

    return Math.min(furthest, A2_TOTAL_BOXES);
};
...
window.a2EnsureActionPanel = function() {
    ...
    panel.innerHTML = `
        ...
        <div class="arena12-current">
            ...
            <div class="arena12-current-name">${safeEsc((window.a2TaskBadge({type: t.type}) ? window.a2TaskBadge({type: t.type}) + ' ' : '') + (t.task_name || 'Atanmadı'))}</div>
        ...
        <button id="arena12-reroll-btn" class="x-btn arena12-reroll">🎲 Görevi Değiştir</button>
    `;

    const btn = document.getElementById('arena12-reroll-btn');
    if (btn) {
        btn.disabled = (!st.isCaptain || st.rerollsLeft <= 0 || !!st.pendingMove || st.isFinished);
        btn.onclick = () => window.openArenaReroll();
    }
};
    ...
    const feed = window.a2ArenaRows().slice().sort((a,b)=>new Date(b?.created_at||0)-new Date(a?.created_at||0)).slice(0,10).map(m => {
        ...
        return `
            <div class="arena12-feed-item ${isPending ? 'pending' : ''}">
                ...
                <div class="arena12-feed-body">${safeEsc(humanNote)}</div>
            </div>
        `;
    }).join('') || '<div class="arena-empty-note">Henüz yönetim kaydı yok.</div>';
    ...
    const res = await Swal.fire({
        ...
        html: `
            <div style="text-align:left">
                ...
                <label style="display:block; margin-bottom:6px;">Ad</label>
                <input id="arena12-special-name" value="Özel Kutu" style="width:100%; padding:10px; border-radius:10px; background:#0f172a; color:#fff; border:1px solid #334155; margin-bottom:10px;" />
                <label style="display:block; margin-bottom:6px;">Adım</label>
                <input id="arena12-special-steps" type="number" min="1" max="5" value="1" style="width:100%; padding:10px; border-radius:10px; background:#0f172a; color:#fff; border:1px solid #334155;" />
            </div>
        `,
        showCancelButton: true,
        confirmButtonText: 'Kaydet',
        ...
        color: '#fff',
        preConfirm: () => ({
            type: document.getElementById('arena12-special-type')?.value || 'reward',
            name: document.getElementById('arena12-special-name')?.value || 'Özel Kutu',
            steps: Math.max(1, Number(document.getElementById('arena12-special-steps')?.value || 1))
        })
    });
    if (!res.isConfirmed) return;
    const v = res.value;
    const ins = await sb.from('competition_config').insert({ task_name: v.name, steps: v.steps, type: v.type }).select();
    if (ins?.error) return Swal.fire('Hata', ins.error.message || 'Özel kutu eklenemedi.', 'error');
    await syncCompetitionData();
    return Swal.fire('Kaydedildi', 'Özel kutu tanımı eklendi.', 'success');
};
...
window.a2CompletedTaskIds = function(teamKey) {
    const done = new Set();
    (window.a2RowsByType ? window.a2RowsByType(teamKey, 'submission') : []).forEach(row => {
        if (String(row?.status || '').toLowerCase() !== 'approved') return;
        const taskId = Number(window.a2TagVal ? window.a2TagVal(row?.admin_note, 'TASK') : 0);
    ...
    window.openArenaReroll = async function() {
        ...
        try {
            ...
            const pool = window.a2CandidateTaskPool().filter(t => {
                ...
                return id !== activeTaskId && !completed.has(id);
            });
...
window.a2OpenReportModal = async function(boxNo) {
    if (window.__arena12SubmitLock) return;
    const st = window.a2CurrentState(currentUser);
    ...
    window.__arena12SubmitLock = true;
    try {
        const note = `[ARENA2][TYPE:submission][TEAMKEY:${st.teamKey}][BOX:${targetBox}][TASK:${task.id}][DESC:${text.slice(0, 100).replace(/\[|\]/g, '')}]`;
        const ins = await sb.from('competition_moves').insert({
            user_name: currentUser,
            task_id: Number(task.id),
            steps: Number(st.currentSegment?.steps || 1),
            admin_note: note,
            status: 'pending'
        });
...
window.handleGrandPrizeClick = function() {
    ...
    if (st.isFinished) {
        const gpRow = (competitionConfig || []).find(c => String(c.task_name).startsWith('[GRANDPRIZE]'));
        const gpText = gpRow ? String(gpRow.task_name).slice(12) : 'Arena 2.0 Macera Haritasını tamamlayarak büyük ödülün sahibi oldunuz.';

        window.a2TriggerVictoryAnimation();
        window.a2PlaySound('victory');
        Swal.fire({
            title: '🏁 Efsanevi Zafer!',
            html: `
                <div style="text-align:center;">
                    <p style="font-size:1.1rem; color:#fde68a;">Tebrikler <b>${currentUser}</b>!</p>
                    <p style="color:#94a3b8;">${safeEsc(gpText)}</p>
                    <div style="font-size:3rem; margin:15px 0;">🏆🏰🔥</div>
                </div>
            `,
            background: '#020617',
            color: '#fff',
            confirmButtonText: 'Harikayım!',
            confirmButtonColor: '#d97706'
        });
    } else {
        Swal.fire({
            title: '🏰 Büyük Ödül Kalesi',
            text: 'Buraya ulaşmak için tüm kutuları tamamlamalısın! Şimdilik sadece uzaktan bakabilirsin...',
            icon: 'lock',
            background: '#020617',
            color: '#fff',
...
window.a2TriggerVictoryAnimation = function() {
    // Simple CSS based confetti if no library available
    for (let i = 0; i < 50; i++) {
...
window.a2StartAutoRefresh = function(seconds = 45) {

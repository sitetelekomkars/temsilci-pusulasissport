...
// 🏆 QuizResults - 5 dakika cache (home her açıldığında istek gitmez)
let _quizResultsCache = null;
let _quizResultsCacheTime = 0;
...
async function fetchLeaderboard(targetTbodyId = 'leaderboard-body', targetLoaderId = 'leaderboard-loader', targetTableId = 'leaderboard-table') {
    ...
    // ✅ 5 dakika cache: Hero lobby her açıldığında QuizResults'e gitmiyoruz
    const now = Date.now();
    let data = null;
    if (_quizResultsCache && (now - _quizResultsCacheTime < 300000)) {
        data = _quizResultsCache;
        console.log('[Pusula] QuizResults cache\'ten döndü (Supabase atlandı).');
    } else {
        if (loader) loader.style.display = 'block';
        if (table) table.style.display = 'none';
        tbody.innerHTML = '';
        try {
            const res = await sb.from('QuizResults').select('*').order('Score', { ascending: false }).limit(20);
            if (res.error) throw res.error;
            data = res.data || [];
            _quizResultsCache = data;
            _quizResultsCacheTime = now;
        } catch (err) {
            console.warn("Leaderboard fetch error:", err);
            if (loader) { loader.innerText = "Yüklenemedi."; loader.style.display = 'block'; }
            return;
        }
    }
...
function finishPenaltyGame() {
    ...
    // Leaderboard log + cache invalidation
    apiCall('logQuiz', {
        ...
        // ♻️ Oyun bitti → cache'i temizle, taze liderlik tablosu çekilsin
        _quizResultsCache = null;
        _quizResultsCacheTime = 0;

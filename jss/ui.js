...
async function fetchBroadcastFlow() {
    // 🧠 60 dakika in-memory cache — home paneli tekrar açılınca istek gitmesin
    const now = Date.now();
    if (window._bfCache && (now - window._bfCache.ts < 3600000)) {
        return window._bfCache.items || [];
    }
    try {
        const res = await apiCall('getBroadcastFlow');
        if (res.result === 'success') {
            window._bfCache = { items: res.items || [], ts: now };
            return res.items || [];
        }
        return [];
    } catch (err) {
        console.error("[Pusula] fetchBroadcastFlow Error:", err);
        return (window._bfCache && window._bfCache.items) || [];
    }
}
...
async function openBroadcastFlow() {
    ...
    const hasCache = window._bfCache && (nowTS - window._bfCache.ts < 3600000); // 60 dakika
    ...
    try {
        ...
        const formatDateLabel = (iso) => {
            ...
            if (iso === tomorrowISO) return { main: "YARIN", sub: dt.toLocaleDateString('tr-TR', { day: 'numeric', month: 'short' }) };
...
async function setBroadcastSheetUrl() {
    ...
    if (urlObj) {
        window._bfCache = null; // Eski önbelleği temizle
